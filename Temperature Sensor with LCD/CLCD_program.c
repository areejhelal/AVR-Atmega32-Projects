...
//basma function
void CLCD_voidWriteNumber(float number) {
	number = ((u32) (number * 100 + .5) / 100.0);
	if(number==0.00){
		CLCD_void_WriteString("0.00");
	}
	else{
	u32 decimal_value = number * 100;
	u8 count = 0;
	u32 temp = decimal_value;
	while (temp > 0) {
		temp = temp / 10;
		count++;
	}
    ...
    int i;
	for (i = count - 1; i >= 0; i--) {
		arr[i] = decimal_value % 10;
		decimal_value = decimal_value / 10;
	}
	for (i = 0; i < count - 2; i++) {
		CLCD_voidWriteData(arr[i]+48);
	}
	CLCD_voidWriteData('.');
